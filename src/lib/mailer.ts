...
export async function sendOrderConfirmationEmail({
  ...
  const itemLines = items
    .map((i) => `・${i.productName}　${i.volume ?? ""}　${i.qty}ケース　¥${i.unitPrice.toLocaleString()}`)
    .join("<br>");

  const html = `<!DOCTYPE html>
<html lang="ja">
<head><meta charset="UTF-8"></head>
<body style="font-family:'Helvetica Neue',Arial,'Hiragino Kaku Gothic ProN',sans-serif;font-size:14px;color:#333;line-height:1.8;padding:24px;">
<p>${customerName} 様</p>
<p>${breweryName}よりご注文が確定しました。</p>
<p>受注番号：${orderNumber}</p>
<p>
${itemLines}
</p>
${adminReply ? `<p>【蔵元からのメッセージ】<br>${adminReply.replace(/\n/g, "<br>")}</p>` : ""}
<p>発注履歴：https://orderlink.jp/portal/orders</p>
<p style="color:#999;font-size:12px;">このメールはOrderLinkから自動送信されています。返信はお受けできません。</p>
</body>
</html>`;
