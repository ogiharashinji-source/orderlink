...
export default function InviteCustomerPage() {
  ...
  const handleImport = async () => {
    ...
    if (res.ok) {
      const data = await res.json();
      setRows(data.results.map((r: ImportRow & { status: string }) => ({ ...r, status: r.status as ImportRow["status"] })));
      setImported(true);
    }
  };
          ...
          {rows.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-gray-700">
                  {imported ? `${rows.filter((r) => r.status === "done").length}件登録完了` : `${rows.length}件読み込みました`}
                </p>
                {imported && (
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {rows.map((row, i) => (
                      <tr key={i}>
                        ...
                        {imported && <td className="px-3 py-2 text-gray-700 font-mono">{row.password || "—"}</td>}
                        <td className="px-3 py-2">
                          ...
                          {row.status === "error" && <span className="text-red-500 text-xs">{row.errorMsg ?? "エラー"}</span>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
