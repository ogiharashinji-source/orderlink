...
export default function PortalChatRoomPage() {
  ...
  const load = useCallback((silent = false) => {
    fetch(`/api/portal/chat?companyId=${companyId}`).then((r) => {
  ...
  const handleSend = async () => {
    ...
    const res = await fetch("/api/portal/chat", {
      ...
      body: JSON.stringify({ companyId, body: text }),
    });
    if (res.ok) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <button onClick={() => router.push("/portal/chat")} className="text-sm text-gray-500 hover:text-blue-600">
          ← チャット一覧
        </button>
      </div>
